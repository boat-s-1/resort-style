...
const blankBooking = { id: '', store_id: '', therapist_id: '', customer_id: '', booking_date: jpDate(), start_time: '', duration_minutes: 90, course_name: '90分コース', base_price: 0, nomination_fee: 0, extension_fee: 0, discount: 0, therapist_payout: 0, payment_method: 'unpaid', status: 'reserved', is_first_visit: true, notes: '' };
...
export default function Operations({ stores, therapists, schedules, notify }) {
  ...
  function exportCsv() {
    const head = ['日付','開始','店舗','女の子','顧客','電話','コース','分','売上','バック','実質売上','支払','状態'];
    const rows = monthRows.map((b) => [b.booking_date,b.start_time?.slice(0,5),b.resort_stores?.name,b.resort_therapists?.name,b.resort_customers?.name,b.resort_customers?.phone,b.course_name,b.duration_minutes,b.total_amount,b.therapist_payout,b.total_amount-b.therapist_payout,b.payment_method,labels[b.status]]);
  ...
  return <section>
    ...
    <div className="admin-table-wrap admin-table-section"><table className="admin-table"><thead><tr>{['日時','店舗・女の子','顧客','コース','売上','状態','操作'].map((h)=><th key={h}>{h}</th>)}</tr></thead><tbody>{monthRows.map((b)=><tr key={b.id}><td>{b.booking_date}<small>{b.start_time?.slice(0,5)}</small></td><td>{b.resort_stores?.name}<small>{b.resort_therapists?.name}</small></td><td>{b.resort_customers?.name}<small>{b.resort_customers?.phone}</small></td><td>{b.course_name}<small>{b.duration_minutes}分</small></td><td>{yen(b.total_amount)}<small>実質 {yen(b.total_amount-b.therapist_payout)}</small></td><td><span className={`admin-badge ${b.status}`}>{b.payment_method==='unpaid'&&b.status==='reserved'?'未会計':labels[b.status]}</span></td><td><div className="admin-row-actions"><button onClick={()=>{setBooking({...blankBooking,...b,store_id:String(b.store_id),start_time:b.start_time?.slice(0,5)||''});scrollTo({top:0,behavior:'smooth'});}}>編集</button><button className="danger" onClick={()=>removeBooking(b.id)}>削除</button></div></td></tr>)}</tbody></table>{!monthRows.length&&<p className="admin-empty">対象月の予約はありません。</p>}</div></>}
    ...
    {view === 'activity' && <Table head={['女の子','接客','施術時間','稼働率','売上','バック','指名','リピート']} rows={activity.map((t)=>[t.name,`${t.count}件`,`${t.minutes}分`,t.utilization===null?'—':`${t.utilization}%`,yen(t.sales),yen(t.payout),`${t.nomination}件`,`${t.repeat}件`])} />}
  </section>;
}
